...
CharacterScreenLeftPanelHeaderModule.prototype.createDismissDialogContent = function (_dialog)
{
    ...
    if (showCheckbox)
    {
        ...
        var checkboxLabel = $('<label class="blub text-font-medium font-color-subtitle font-style-normal" for="compensation">Pay <img src="' + Path.GFX + Asset.ICON_MONEY_SMALL + '"/>' + selectedBrother['compensationCost'] + ' ' + paymentVerb + '</label>');
    ...
    {
        self.mPayDismissalWage = true;
        retirementPackage.append($('<div class="label text-font-medium font-color-subtitle font-style-normal">Pay <img src="' + Path.GFX + Asset.ICON_MONEY_SMALL + '"/>' + selectedBrother['compensationCost'] + ' ' + paymentVerb + '</div>'));
    }
    ...
    {
        breakdownLines = ['Final compensation: ' + selectedBrother['compensationCost']];
    }

    for (var i = 0; i < breakdownLines.length; ++i)
    {
        breakdownContainer.append($('<div class="label text-font-small font-color-description font-style-normal">' + breakdownLines[i] + '</div>'));
    }
